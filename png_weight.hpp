...
void weight_png(const std::string& filename, const T& weight,
  size_t img_width, size_t img_height, size_t padding_x, size_t padding_y) {
    ...
    const mat_t nb_weight = no_bias(weight);
    const size_t num_outputs = nb_weight.n_rows;
    const size_t num_inputs = nb_weight.n_cols;
    ...
    const elem_type min = nb_weight.min();
    const elem_type max = nb_weight.max();
    const elem_type bias = -1.0 * min;
    const elem_type scale = 1.0 / (max - min);

    for (size_t img = 0; img < nb_weight.n_rows; ++img) {
        ...
        // Draw the weight
        draw_weight(png, nb_weight.row(img), x, y, img_width, img_height, scale, bias);
    }
